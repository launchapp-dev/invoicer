...
| Result | PASS WITH WORKAROUND — App at port 3002 (port 3000 = 500). No TASK-329 recurrence. Login ✓ (qa-test@invoicer.dev, existing account). TASK-330 PERSISTS: fiber workaround applied. NEW DETAIL: lineItems setValue now requires `amount: qty*rate` field — omitting it yields $NaN subtotal (form calculates subtotal from item.amount, not qty×rate live). Invoice saved ✓ (INV-017, ID: 78820063-...). Subtotal $1,500.00 ✓. Preview ✓ (Test Company, Client Corp, Consulting 10×$150=$1,500.00). Download PDF ✓ (INV-017.pdf). All 8 authenticated routes 200. Logout ✓ (→ /login). 0 console errors. 0 network errors. |
| Steps Passed | 6/6 |
| Steps Failed | 0/6 |
| Console Errors | 0 |
| Network Errors | 0 |
