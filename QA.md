...
| Field | Value |
|-------|-------|
| Date | 2026-03-29 (run 32) |
| Result | PASS WITH WORKAROUND — Signup ✓ (qa-test32@invoicer.dev, new account). Dashboard ✓ ($0.00 new user, Cash Flow Forecast widget, invoice list). Invoice form ✓ — CRITICAL: TASK-330 persists. text fields reject keyboard/fill input. Workaround: walk fiber tree via memoizedProps, find RHF form with setValue+register, call setValue() directly. All values set: from.name=Test Company, to.name=Client Corp, description=Consulting, qty=10, rate=150. Subtotal $1,500.00 correct in form, preview, and dashboard (qty/rate spinbuttons work; numeric amounts calculated correctly). Invoice save ✓ (redirects to /invoices/:id). Preview ✓ ($1,500.00, all data correct). PDF download ✓ (INV-001.pdf, 0 errors). All 8 routes 200 (/dashboard, /invoices/new, /clients, /expenses, /dashboard/recurring, /settings, /settings/templates, /login). Logout ✓ (→ /login). 0 console errors. 0 network errors. NOTE: Subtotal correctly shows $1,500.00 this run (improvement vs run 31 $0.00) — spinbutton fill sets numeric values and triggers onChange correctly. |
| Steps Passed | 6/6 |
| Steps Failed | 0/6 |
| Console Errors | 0 |
| Network Errors | 0 |
...
- ~~**[2026-03-29] CRITICAL: better-sqlite3 NODE_MODULE_VERSION mismatch — TASK-329** — RESOLVED run 21 (Node v22.17.0, disk space freed). Recurring issue: always start with `nvm use 22 && pnpm dev`. Long-term fix needed: add `.nvmrc` pinning Node v22.~~
