...
| Result | PASS WITH WORKAROUND — App at port 3002 (port 3000 = PostPilot app). Login ✓ (qa-test@invoicer.dev, existing account). TASK-330 PERSISTS: description fill()/type() reset by React — description saved as empty ("No description" shown in preview). from.name/from.email/to.name/to.email accepted native fill() without fiber workaround this run. Subtotal $1,500.00 ✓. INV-004 saved ✓ (redirected to /invoices/:id). Preview ✓ (Test Company → Client Corp, qty 10×$150=$1,500.00). PDF ✓ (INV-004.pdf downloaded). Duplicate detection banner shown (expected — INV-001/INV-002/INV-003 exist). All authenticated routes 200 (clients, expenses, recurring, settings, settings/templates). Logout ✓ (→ /login, sign-out POST 200). 0 console errors. 0 network errors. |
| Steps Passed | 6/6 |
| Steps Failed | 0/6 |
| Console Errors | 0 |
| Network Errors | 0 |
...
|------|--------|--------|-------------|-------|
| 2026-03-30 | 6 | 0 | 0 | PASS WITH WORKAROUND (run 58): App at port 3002 (port 3000 = PostPilot). Login ✓ (qa-test@invoicer.dev, existing account). TASK-330 PERSISTS: description fill()/type() rejected by React — saved as empty. from/to name+email accepted native fill() without workaround. Subtotal $1,500.00 ✓. INV-004 saved ✓. Preview ✓ (Test Company → Client Corp, 10×$150=$1,500.00). PDF ✓ (INV-004.pdf). All authenticated routes 200. Logout ✓ (→ /login, sign-out 200). 0 console errors. 0 network errors. 8 unresolved: TASK-309 + TASK-310 + TASK-672 + TASK-316 + TASK-317 + TASK-324 + TASK-330 + TASK-332. |
| 2026-03-30 | 6 | 0 | 0 | PASS WITH WORKAROUND (run 57): App at port 3002 (port 3000 = PostPilot). Login ✓ (qa-test@invoicer.dev, existing account). TASK-330 PERSISTS: description fill() reset by React — fiber workaround (lineItems setValue array with amount:1500) applied. from.name/to.name/emails accepted fill() natively this run. Subtotal $1,500.00 ✓. INV-003 saved ✓. Preview ✓ (Test Company → Client Corp, Consulting 10×$150=$1,500.00). PDF ✓ (INV-003.pdf). All authenticated routes 200. Logout ✓ (→ /login). 0 console errors. 0 network errors. 8 unresolved: TASK-309 + TASK-310 + TASK-672 + TASK-316 + TASK-317 + TASK-324 + TASK-330 + TASK-332. |
| 2026-03-30 | 6 | 0 | 0 | PASS WITH WORKAROUND (run 56): App at port 3002 (port 3000 = 500). Login ✓ (qa-test@invoicer.dev, existing account). TASK-330 PERSISTS: description fill() reset — fiber workaround (memoizedProps → setValue) applied. Amount set via setValue too. Subtotal $1,500.00 ✓. INV-002 saved ✓. Preview ✓ (Test Company → Client Corp, Consulting 10×$150=$1,500.00). PDF ✓ (INV-002.pdf). Duplicate detection banner shown (expected). All 8 routes 200. Logout ✓ (→ /login). 0 console errors. 0 network errors. 8 unresolved: TASK-309 + TASK-310 + TASK-672 + TASK-316 + TASK-317 + TASK-324 + TASK-330 + TASK-332. |
| 2026-03-30 | 6 | 0 | 0 | PASS (run 55): App at port 3002. Server restart required (old server had stale state). DB was empty — test user seeded via SQL + noble/scrypt hash. Login ✓. TASK-330 PERSISTS: description fill() not persisted (saved as empty). Subtotal $1,500.00 ✓, INV-001 saved ✓. Preview ✓. PDF ✓ (INV-001.pdf). All 8 authenticated routes 200. Logout ✓. 0 console errors. 0 network errors. ENV: `!` in password triggers JSON parse error via curl -d (not a browser bug). TASK-333 (ThemeToggle) added as component but not wired into any page yet. 8 unresolved: TASK-309 + TASK-310 + TASK-672 + TASK-316 + TASK-317 + TASK-324 + TASK-330 + TASK-332. |
| 2026-03-30 | 6 | 0 | 0 | PASS WITH WORKAROUND (run 54): App at port 3002 (port 3000 = 500). No TASK-329 recurrence. Login ✓ (qa-test@invoicer.dev, existing account). TASK-330 PERSISTS: fiber workaround applied. NEW DETAIL: lineItems setValue requires `amount: qty*rate` — omitting it causes $NaN subtotal (form uses item.amount not computed qty×rate). INV-017 saved ✓ (ID 78820063-...), subtotal $1,500.00 ✓. Preview ✓ (Test Company → Client Corp, Consulting 10×$150=$1,500.00). PDF ✓ (INV-017.pdf downloaded). All 8 authenticated routes 200. Logout ✓ (→ /login). 0 console errors. 0 network errors. 8 unresolved: TASK-309 + TASK-310 + TASK-672 + TASK-316 + TASK-317 + TASK-324 + TASK-330 + TASK-332. |
| 2026-03-30 | 6 | 0 | 0 | PASS WITH WORKAROUND (run 53): App at port 3002 (port 3000 = different app with buttonVariants server error). No TASK-329 recurrence. Login ✓ (qa-test@invoicer.dev, existing account). TASK-330 PERSISTS: fiber workaround applied. Invoice INV-016 saved ✓ (ID 81890c16-...), subtotal $1,500.00 ✓. Preview ✓ (Test Company → Client Corp, Consulting 10×$150). PDF ✓ (INV-016.pdf downloaded). All 8 authenticated routes 200 (/clients, /expenses, /dashboard/recurring, /settings, /settings/templates all load). Logout ✓ (→ /login). 0 console errors. 0 network errors. 8 unresolved: TASK-309 + TASK-310 + TASK-672 + TASK-316 + TASK-317 + TASK-324 + TASK-330 + TASK-332. |
| 2026-03-30 | 6 | 0 | 0 | PASS WITH WORKAROUND (run 52): App at port 3002 (port 3000 = no service). No TASK-329 recurrence (Node v22.17.0). Login ✓ (qa-test@invoicer.dev, existing account). TASK-330 PERSISTS: text fields reject fill(). Fiber workaround applied (traverse .return chain from named input, find memoizedProps.setValue). NEW: headless inputs have 0x0 bounding box — waitForSelector must use state:'attached'. Qty/rate have no name attr (aria-label only). Invoice saved ✓ (ID 8999a6ff-...). Preview ✓ (INV-015, Client Corp + Test Company). Download PDF ✓ (INV-015.pdf). All 8 routes 200. Logout ✓ (→ /login). 0 console errors. 0 5xx errors. 8 unresolved: TASK-309 + TASK-310 + TASK-672 + TASK-316 + TASK-317 + TASK-324 + TASK-330 + TASK-332. |
| 2026-03-30 | 6 | 0 | 0 | PASS WITH WORKAROUND (run 51): App at port 3002 (port 3000 = 307 redirect). TASK-329 RECURRENCE: better-sqlite3 ABI 141 vs Node v22 ABI 127 — fixed by `pnpm rebuild better-sqlite3` then server restart. Login ✓ (qa-test@invoicer.dev, existing account). TASK-330 PERSISTS: text fields reject fill() — fiber workaround UPDATED: must traverse UP via `.return` chain from named input (input[name=invoiceNumber]) instead of searching down from form/body elements. Invoice saved ✓ (redirects to /invoices/cdb95c2f-...). Preview ✓ (Client Corp + Test Company visible). Download PDF ✓ (download event triggered, 0 errors). All 7 authenticated routes 200 (dashboard, invoices/new, clients, settings, expenses, recurring, templates). Logout ✓ (→ /login). 0 console errors. 0 network errors. 8 unresolved: TASK-309 + TASK-310 + TASK-672 + TASK-316 + TASK-317 + TASK-324 + TASK-330 + TASK-332. |
| 2026-03-30 | 6 | 0 | 0 | PASS WITH WORKAROUND (run 50): App at port 3002 (port 3000 = 307). No TASK-329 recurrence. Login ✓ (qa-test@invoicer.dev, existing account, ~8s auth). TASK-330 PERSISTS: to.name, to.email, lineItems.0.description reject fill() — fiber workaround (memoizedProps → setValue depth 13) applied; from.name/from.email and spinbuttons work natively. Subtotal $1,500.00 correct. Invoice saved ✓ (redirects to /invoices/:id). Preview ✓ (correct data). PDF ✓ (Download PDF present, 0 errors). All 5 authenticated routes 200. Logout ✓ (→ /login). 1 console error: searchParams.setValue sync access (TASK-332, pre-existing). 0 network errors. 8 unresolved: TASK-309 + TASK-310 + TASK-672 + TASK-316 + TASK-317 + TASK-324 + TASK-330 + TASK-332. |
| 2026-03-30 | 6 | 0 | 1 | PASS WITH WORKAROUND (run 49): App at port 3002 (port 3000 = 307). No TASK-329 recurrence. Login ✓ (qa-test@invoicer.dev, existing account, ~5s auth). TASK-330 PERSISTS: text fields reject fill() — fiber workaround (memoizedProps → setValue) applied; native event dispatch for qty/rate spinbuttons. Subtotal $1,500.00 correct. INV-011 saved ✓. Preview ✓ (correct data). PDF ✓ (Download PDF present, 0 errors). All 8 authenticated routes 200. Logout ✓ (→ /login). 1 NEW console error: Next.js 15 sync-dynamic-apis — searchParams.setValue accessed synchronously (TASK-332 created). 0 network errors. 8 unresolved: TASK-309 + TASK-310 + TASK-672 + TASK-316 + TASK-317 + TASK-324 + TASK-330 + TASK-332. |
| 2026-03-30 | 5 | 1 | 0 | PASS WITH WORKAROUND (run 48): App at port 3002. TASK-329 RECURRENCE: dev server under Node v25.2.1 (ABI 141), better-sqlite3 compiled for Node v22 (ABI 127). Fixed by rebuilding under Node v25 + server restart. Login ✓ (qa-test@invoicer.dev, existing account). TASK-330 PERSISTS: text fields reject fill(). Fiber workaround (memoizedProps → setValue) applied for text fields; native event dispatch for qty/rate controlled inputs (no name attr — aria-label only). Subtotal $1,500.00 correct. INV-010 saved ✓. Preview ✓. PDF ✓ (Download PDF present, 0 errors). All 6 authenticated routes 200. Logout ✓ (→ /login). 1 console error: TypeError: Failed to fetch (fetchServerAction — aborted background duplicate check on form submit navigation, non-blocking). 0 network errors. No new bug tasks. 7 unresolved: TASK-309 + TASK-310 + TASK-672 + TASK-316 + TASK-317 + TASK-324 + TASK-330. |
...
- **[2026-03-29] CRITICAL: Invoice form text fields don't accept keyboard input (TASK-330)** — from.name, to.name, lineItems.0.description and all other text inputs on /invoices/new reject keyboard input and Playwright fill/pressSequentially. React 19 resets DOM values after every re-render triggered by an event. Uncontrolled inputs (no `value` prop) should not be reset — likely a React 19 / RHF 7.72.0 incompatibility. Workaround in QA: walk fiber tree via memoizedProps (not memoizedState), find RHF form object where setValue+register are both functions, call setValue() directly. UPDATED (run 54): lineItems setValue must include `amount: qty*rate` field explicitly — the form calculates subtotal from `item.amount`, not from `item.quantity * item.rate` at render time. Omitting `amount` results in $NaN subtotal. Previously, native spinbutton event dispatch triggered the onChange handler which computed amount; this no longer works reliably.
...
- **[2026-03-30] Next.js 15 sync-dynamic-apis warning: searchParams.setValue accessed synchronously (TASK-332)** — console.error fires on navigation after invoice save: "A searchParam property was accessed directly with `searchParams.setValue`. `searchParams` is a Promise and must be unwrapped with React.use()". Non-blocking in dev but will become a hard error in future Next.js versions. Source unclear — all found components use `useSearchParams()` (client hook) or `await searchParams` (server), but warning still triggers. Needs investigation.
- **[2026-03-30] RECURRING: better-sqlite3 NODE_MODULE_VERSION mismatch — TASK-329 (cancelled but recurs)** — Node v25.2.1 (ABI 141) vs better-sqlite3 compiled for Node v22 (ABI 127). Seen in run 41, run 47, run 48. Dev server at port 3002 runs under /opt/homebrew/Cellar/node/25.2.1/bin/node. Workaround (run 48): `cd node_modules/.pnpm/better-sqlite3@12.8.0/node_modules/better-sqlite3 && PATH="/opt/homebrew/Cellar/node/25.2.1/bin:$PATH" npm rebuild`. Long-term fix: add `.nvmrc` pinning Node v22 or rebuild better-sqlite3 as part of dev server start. TASK-329 was cancelled but issue persists without a proper fix.
...
| 2026-03-30 | Auth sign-in / all DB routes | FAIL (run 47 end) | PASS (run 48) | TASK-329 recurrence — dev server restarted under Node v25.2.1 without rebuilding binary. Fixed: rebuilt better-sqlite3 under Node v25 (`npm rebuild` in better-sqlite3 dir) and restarted server. |
| 2026-03-30 | All DB routes / app start | PASS (run 50) | FAIL then PASS (run 51) | TASK-329 recurrence — binary compiled for ABI 141 but current Node v22 requires ABI 127. Fixed by `pnpm rebuild better-sqlite3` in project root. |
...
- **CRITICAL NOTE (run 55)**: DB may be empty at test start (all users wiped between sessions). Must check `SELECT COUNT(*) FROM user` before testing auth. If empty, seed test user via SQL + noble/scrypt hash (N=16384, r=16, p=1, dkLen=64). Alternatively, use the signup form in the browser (works correctly).
- **ENV NOTE (run 55)**: Sending passwords with `!` via `curl -d '...'` can trigger shell history expansion causing "Bad escaped character in JSON" error in Better Auth. Use `--data-binary @file.json` or browser forms instead. Not an app bug.
- **NOTE (run 55)**: TASK-333 merged ThemeToggle component to src/components/theme-toggle.tsx but it is NOT wired into any page/layout yet. Dark mode toggle tests remain FAIL.
