...
| Result | PASS — App at port 3002. Server restarted (old PID 93127 killed, new server started). DB empty at test start — test user seeded directly via SQL + better-auth scrypt hash. Login ✓ (qa-test@invoicer.dev, seeded). TASK-330 PERSISTS: description field fill() doesn't persist — saved as empty "No description". Subtotal $1,500.00 ✓. INV-001 saved ✓. Preview ✓ (Test Company → Client Corp, 10×$150=$1,500.00). PDF ✓ (INV-001.pdf). All 8 routes 200. Logout ✓ (→ /login). 0 console errors. 0 network errors. NEW ENV NOTE: `!` in password causes Better Auth JSON parse error when sent via `curl -d` (shell history expansion) — works fine via browser. |
| Steps Passed | 6/6 |
| Steps Failed | 0/6 |
| Console Errors | 0 |
| Network Errors | 0 |
